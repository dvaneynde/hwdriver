...
public class Main {
    ...
    public void runDomotic(final Domotic dom, final Oscillator osc, String pathToDriver) {
        if (pathToDriver == null) {
            ...
            // TODO driver in aparte thread, main loop doet osc.go() en kijkt of alles nog draait - driver en domotic; zoniet herstart of exit;
            ...
            try {
                ...
                t.start(); // TODO wacht op klaar lijn in driver log hieronder, dan pas domotic starten
     ...
     * <ol>
     * <li>The Hardware Listener, listening for incoming messages from the C hardware driver, and returning commands.</li>
     * <li>The Hardware Driver, a C program.</li>
     * <li>The RMI server, for a Swing client, or JBoss server (for web site later on).</li>
     * </ol>
