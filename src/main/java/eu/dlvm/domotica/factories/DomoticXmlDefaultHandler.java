...
class DomoticXmlDefaultHandler extends DefaultHandler2 {
    ...
    public void startElement(String uri, String localName, String qqName, Attributes atts) throws SAXException {
        if (localName.equals("domotic")) {
            ...
            // TODO introduce interace ICapabilitySwitchConnectable to avoid below if/else, also update parseConnection() and rename parseSwitchConnection
    ...
    private void parseAllSwitch(Block sb, Attributes atts) {
        ...
        if (!blocks.containsKey(switchName)) {
            throw new ConfigurationException("SwitchBoard " + sb.getName() + " needs a Switch named " + switchName
    ...
    private void parseConnection(SwitchBoard sb, Attributes atts) {
        ...
        if (!blocks.containsKey(lampName)) {
            throw new ConfigurationException("SwitchBoard " + sb.getName() + " needs a Lamp named " + lampName
                    + ", which it did not encounter in configuration file.");
        }
    ...
    private void parseConnection(SwitchBoardFans sb, Attributes atts) {
        ...
        if (!blocks.containsKey(switchName)) {
            throw new ConfigurationException("SwitchBoardFans " + sb.getName() + " needs a Switch named " + switchName
                    + ", which it did not encounter in configuration file.");
        }
        if (!blocks.containsKey(fanName)) {
            throw new ConfigurationException("SwitchBoard " + sb.getName() + " needs a Fan named " + fanName
                    + ", which it did not encounter in configuration file.");
        }
    ...
    private void parseConnection(SwitchBoardDimmers sb, Attributes atts) {
        ...
        if (!blocks.containsKey(switchName)) {
            throw new ConfigurationException("SwitchBoardDimmers " + sb.getName() + " needs DimmerSwitches named " + switchName
                    + ", which it did not encounter in configuration file.");
        }
        if (!blocks.containsKey(lampName)) {
            throw new ConfigurationException("SwitchBoard " + sb.getName() + " needs a Lamp named " + lampName
