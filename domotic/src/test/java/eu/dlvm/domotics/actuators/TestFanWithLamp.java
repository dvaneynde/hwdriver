...
public class TestFanWithLamp {
	...
	public void init() {
		...
		lamp = new Lamp("TestLamp", "TestLamp", false, LAMP_OUT, hw, ctx);
		fan = new Fan("TestFanWithLamp", "TestFanWithLamp", FAN_OUT, hw, ctx).overrideDelayOff2OnSec(5).overrideDelayOn2OffSec(5)
	...
	public void manuallyTurnOnFanAndLetItTimeout() {
		...
		fan.loop(current += 10, seq++);
		fan.loop(current += fan.getDelayOff2OnSec() * 1000, seq++);
		fan.loop(current += 10, seq++);
		assertOff();
	}
	...
	public void lampNotLongEnoughOnForFanToRun() {
		...
		fan.loop(current += (fan.getDelayOff2OnSec() - 20), seq++);
		assertDelayedOn_LampOn();
